<?php
    ...
    try {
        ...
        if (!empty($_GET['id'])) {
            ...
            $verificar_cliente = "SELECT * FROM paquete_correo WHERE id_correo = ?";
            if ($stmt_verificar = $conn->prepare($verificar_cliente)) {
                ...
                if ($stmt_verificar->num_rows > 0) {
                    ...
                    $eliminar_cliente = "CALL eliminar_paquete_correo(?)";
            ...
            echo json_encode(['success' => false, 'message' => 'ID del Correo no proporcionado']);
        }
        exit();
    } catch (Exception $e) {
