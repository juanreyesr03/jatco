...
-- Procedimientos
--
CREATE DEFINER=`prueba_beta`@`%` PROCEDURE `buscar_parte` (IN `p_numero_parte` VARCHAR(255), IN `p_nombre` VARCHAR(255))   BEGIN
    SELECT 
        numero_parte, 
        nombre
    FROM 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `editar_parte` (IN `p_id_numero_parte` INT, IN `p_numero_parte` VARCHAR(250), IN `p_nombre` VARCHAR(250), IN `p_id_estado` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminar_parte` (IN `p_id_numero_parte` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_parte` (IN `p_numero_parte` VARCHAR(250), IN `p_nombre` VARCHAR(250))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `mostrar_partes` ()   BEGIN
	select 
		s.id_numero_parte,
        s.numero_parte,
        s.nombre,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `mostrar_partes_id` (IN `p_parte_id` INT)   BEGIN
	SELECT 
		id_numero_parte,
        numero_parte, 
        nombre,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `mostrar_partes_numero` (IN `p_numero_parte` VARCHAR(250))   BEGIN
	SELECT 
		id_numero_parte,
        numero_parte, 
        nombre,
